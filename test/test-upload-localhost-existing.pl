...
open(F1, '| perl ../cli/upload.pl http://127.0.0.1:8669 bundle 3.0.z ./upload/bundle3 > /dev/null');
...
open(F1, '| perl ../cli/manage-group.pl http://127.0.0.1:8669 test1 add '.$test_user1.' > /dev/null');
open(F2, '| perl ../cli/manage-group.pl http://127.0.0.1:8669 test1 add '.$test_user2.' > /dev/null');
close(F1);
close(F2);

open(F1, '| perl ../cli/manage-group.pl http://127.0.0.1:8669 test1 remove '.$test_user2.' > /dev/null');
close(F1);
...
die "$user missing from group test1 (",$groups->{test1}->[0],"), stopped" unless $groups->{test1}->[0] eq $user.'@yahoo.com';
die "$test_user1 missing from group test1 (",$groups->{test1}->[1],"), stopped" unless $groups->{test1}->[1] eq $test_user1.'@yahoo.com';
...
die "$user missing from group test2 (",$groups->{test2}->[0],"), stopped" unless $groups->{test2}->[0] eq $user.'@yahoo.com';
